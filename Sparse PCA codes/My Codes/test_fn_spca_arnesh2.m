...
function [f_s,f_n,grad_f_s,prox_psi_n,psi_s,psi_n, grad_psi_s, Lagrangian, params] = test_fn_spca_arnesh2(b, nu, p, n, s, k, seed)
...
prox_psi_n = @(x, lam, L) [kf_proj(x(1:p, :), k); prox_l1(x((p + 1):(2 * p), :), L*lam * nu)];
oracle = Oracle(f_s, f_n, grad_f_s, prox_psi_n);
%%%% mine
psi_s=@(x,lambda,pen,p,w) lambda*(f_s(x)+prod_fn(p,A*x-params.set_projector(1))+(pen/2)*(norm_fn(A*x-params.set_projector(1)))^2)+0.5*(norm_fn(x-w))^2;
psi_n=@(x,lambda) lambda*f_n(x);
grad_psi_s=@(x,lambda,pen,p,w)lambda*(grad_f_s(x)+(transpose(A)*p)+pen*transpose(A)*(A*x-params.set_projector(1)))+(x-w);
Lagrangian=@(pen,z,p) f_s(z)+f_n(z)+params.prod_fn(p,A*z-params.set_projector(1))+(pen/2)*(params.norm_fn(A*z-params.set_projector(1)))^2;
